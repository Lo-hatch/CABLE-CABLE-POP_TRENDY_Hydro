...
Create mask for Cable with only latitude,longitude set to 1 on given
...
--------
  # extract Hawaii
  python extract_restart_point.py glob_ipsl_1x1.nc *_rst_*.nc pop_*_ini_*.nc \
      -l 19.8968,-155.5828
...
Modified Matthias Cuntz, Apr 2020
             - updated to same structure as other netcdf utilities
...
parser = argparse.ArgumentParser(
    ...
    description=('Create mask for Cable with only latitude,longitude set'
                 ' to 1 on given latitudes,longitudes, .\nExtract the'
                 ' latitudes,longitudes from any number of Cable-POP'
                 ' restart files.\n\nOutput files will be named'
                 ' input_name-extract.nc.\n') )
...
parser.add_argument('ifiles', nargs='*', default=None, metavar='netcdf',
                    help=('input netcdf files (>=1): mask'
                          ' [[restart_file1 [restart_file2 ...]]') )
...
if ifiles is None:
    raise IOError('At least mask input file needed. Input files:'
                  ' mask_file restart_file1 restart_file2 ...')
elif len(ifiles) == 1:
    restartfiles = []
else:
...
if 'land' not in fi.variables:
    ...
    print('Land variable must be in first input, which must be the mask'
          ' file: ' + ifile)
    import sys
    sys.exit()
if izip:
...
for ifile in restartfiles:
    ...
    if 'mp' in fi.dimensions.keys():
        # cable file is different to other restart files casa, LUC, climate,
        #   c13o2 lats/lons are given per grid cells but most output is given
        #   per tile-> make lats/lons per tile with the help of number of
        #   tiles per grid cell 'nap'
        ...
        for ii in range(nlatlon):
            ...
            miiland.extend(np.where((np.array(mlats) == mlats[miilat0]) &
                                    (np.array(mlons) == mlons[miilon0]))[0])
        if len(miiland) == 0:
            print('-> no land point.')
            fi.close()
            continue
    ...
    for ii in range(nlatlon):
        ...
        tiiland.extend(np.where((np.array(tlats) == tlats[tiilat0]) &
                                (np.array(tlons) == tlons[tiilon0]))[0])
    if len(tiiland) == 0:
    ...
    for ivar in fi.variables.values():
        ovar = fo.variables[ivar.name]
        if ( ('mp' in ivar.dimensions) or
             ('land' in ivar.dimensions) or
             ('nland' in ivar.dimensions)):
            if len(ivar.shape) == 1:
                ovar[:] = ivar[tiiland]
            else:
                ovar[:] = ivar[..., tiiland]
