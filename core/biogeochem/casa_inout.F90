...
END SUBROUTINE put_casa_var_grid_patch_average
  ...
  subroutine write_casa_output_patch_nc(veg, casamet, casapool, casabal, casaflux, casaonly, ctime, lfinal)
...
!         nf90_def_dim, nf90_unlimited, nf90_int, nf90_def_var, nf90_float, nf90_enddef, nf90_put_var, nf90_close ! , nf90_double
    ...
    integer :: status
    integer :: land_id, x_id, y_id, patch_id, plant_id, litter_id, soil_id, t_id, i
    integer :: llon_id, llat_id
    character(len=99) :: fname
    character(len=50) :: dum
    logical, save :: call1 = .true.
    integer, parameter :: sp = kind(1.0)
    ...
    if ( call1 ) then
       ...
       ! define dimensions:
       ! land (number of points)
       status = nf90_def_dim(file_id, 'land',    mland,         land_id)
       if (status /= nf90_noerr) call handle_err(status)
       status = nf90_def_dim(file_id, 'x', xdimsize, x_id)
       if (status /= nf90_noerr) CALL handle_err(status)
       status = nf90_def_dim(file_id, 'y', ydimsize, y_id)
       ...
       status = nf90_def_var(file_id, 'local_lat', nf90_float, (/land_id/), llat_id)
       if (status /= nf90_noerr) call handle_err(status)
       status = nf90_put_att(file_id, llat_id, 'units', 'degrees_north')
       if (status /= nf90_noerr) call handle_err(status)

       status = nf90_def_var(file_id, 'local_lon', nf90_float, (/land_id/), llon_id)
       if (status /= nf90_noerr) call handle_err(status)
       status = nf90_put_att(file_id, llon_id, 'units', 'degrees_north')
       if (status /= nf90_noerr) call handle_err(status)
       ...
       do i=1, na1
          status = nf90_def_var(file_id, trim(a1(i)), nf90_float, (/land_id,patch_id,t_id/), vid1(i) &
#ifndef __NETCDF3__
               , deflate_level=4, shuffle = .TRUE. &
#endif
               )
  ...
  SUBROUTINE put_casa_var_patch(file_id,var_id,var_vals,cnt,tmp_array)
   ...
   ! write data to file
   status = nf90_put_var(file_id, var_id, tmp_array(:, :, 1), &
                        start=(/1, 1, cnt/), count=(/mland, max_vegpatches, 1/))
                        
   if(status /= nf90_noerr) call handle_err(status)

END SUBROUTINE put_casa_var_patch
